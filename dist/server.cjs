          ...
          responseSchema: {
            ...
            items: {
              ...
              properties: {
                ...
                description: { type: import_genai.Type.STRING },
                expectedEffect: { type: import_genai.Type.STRING },
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: import_genai.Type.OBJECT,
            properties: {
              noveltyScore: { type: import_genai.Type.NUMBER },
              feasibilityScore: { type: import_genai.Type.NUMBER },
              economicsScore: { type: import_genai.Type.NUMBER },
              sustainabilityScore: { type: import_genai.Type.NUMBER },
              totalScore: { type: import_genai.Type.NUMBER },
              pros: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              cons: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } },
              ...
              benchmarks: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    type: { type: import_genai.Type.STRING },
                    keyQuestion: { type: import_genai.Type.STRING },
...
app.post("/api/gemini/cofounder-chat", async (req, res) => {
  try {
    ...
    const ai = getGeminiClient();
    if (ai) {
      const chatHistory = (conversationHistory || []).filter((m) => m.sender === "user" || m.sender === "cofounder").map((m) => ({
...
function generateSmartCofounderFallback(userMsg, ideaTitle = "\uC544\uC774\uB514\uC5B4", role = "\uAE30\uC220 \uD30C\uD2B8\uB108", history = []) {
...
function getFallbackMindmap(title, category, answers) {
...
function getFallbackIdeas(title, category, keywords) {
  ...
  return [
    ...
    },
    {
      ...
      description: "\uC628\xB7\uC624\uD504\uB77C\uC778 \uBAA8\uB4C8\uC744 \uACB0\uD569\uD558\uC5EC \uAE30\uC874 \uC2DC\uC2A4\uD15C\uC5D0 \uBD80\uB4DC\uB7FD\uAC8C \uC5F0\uB3D9\uB418\uB294 \uD601\uC2E0\uC801\uC778 \uC13C\uC11C/SaaS \uD1B5\uD569 \uC194\uB8E8\uC158\uC785\uB2C8\uB2E4.",
...
function getFallbackEvaluation(ideaTitle, category) {
  return {
    noveltyScore: 88,
    feasibilityScore: 82,
    economicsScore: 85,
